...
func (h Handler) DownloadDraftContactNameCard(c *gin.Context) {
	...
	if err != nil {
		log.For(c).Error("[download-draft-contact-name-card] query draft contact info", log.Field("uuid", uuid), 
	...
	if err != nil {
		log.For(c).Error("[download-draft-contact-name-card] query user info", log.Field("uuid", uuid), log.Field("user_id", userID), log.Err(err))
		c.JSON(http.StatusInternalServerError, err)
		return
	}

	if user.OrganizationID != data.OrganizationID {
		log.For(c).Error("[download-draft-contact-name-card] organization id not match", 
	...
	log.For(c).Info("[download-draft-contact-name-card] download", log.Field("uuid", uuid), 
