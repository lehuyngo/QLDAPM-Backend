...
func (h Handler) DeleteContactNote(c *gin.Context) {
	...
	if err != nil {
		log.For(c).Error("[delete-contact-note] query contact note failed", log.Field("user_id", userID), log.Field("note_uuid", noteUUID), log.Err(err))
		c.JSON(http.StatusInternalServerError, err)
		return
	}

	if note.ContactID != contact.ID {
		log.For(c).Error("[delete-contact-note] query contact id isnot match", log.Field("user_id", userID),
	...
	if err != nil {
		log.For(c).Error("[delete-contact-note] query user info failed", log.Field("user_id", userID), log.Field("contact_uuid", contactUUID), log.Err(err))
		c.JSON(http.StatusInternalServerError, err)
		return
	}
	...
	err = h.ContactNote.Delete(ctx, user, contact.ID, note.GetID())
	if err != nil {
